...
class SafePassApp:
    ...
    def show_login_screen(self):
        ...
        def do_login():
            ...
            if res == "SUCCESS":
                ...
                # Download vault blob — may be None if user is brand new
                data = client.server_request("download", user)
                if isinstance(data, bytes) and len(data) > 0:
                    # Valid encrypted vault from server — save locally and load it
                    storage.save_vault_raw(user, data)

                # Load vault from local file, or start fresh if nothing exists
                try:
    ...
    def refresh_vault(self):
        """Downloads the latest version from server and reloads the UI."""
        try:
